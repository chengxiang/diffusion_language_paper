% !TEX root = ../main.tex
\begin{table}[t]
\caption{ELF-L schedule comparison at 32 steps, seeds 42/123. Entries are mean pass@1 (\%). The two epoch-21 columns use supervised states; NFT columns use the selected post-training endpoints.}
\label{tab:schedules}
\centering\small
\begin{tabular}{lrrrr}
\toprule
Layer clocks & GSM8K e21 & GSM8K NFT & MATH e21 & MATH NFT \\
\midrule
$(1.0,1.0,1.0)$ & 52.96 & 58.72 & 15.50 & 20.90 \\
$(1.5,1.5,1.5)$ & 57.35 & 62.32 & 18.10 & 23.50 \\
$(2.0,2.0,2.0)$ & 57.51 & 62.81 & 17.40 & 20.40 \\
$(2.5,2.5,2.5)$ & 55.80 & 61.41 & 18.10 & 20.20 \\
$(2.5,2.0,1.5)$ & 59.25 & 62.47 & 17.90 & 23.00 \\
$(1.5,2.0,2.5)$ & 58.07 & 62.85 & 17.60 & 22.20 \\
\bottomrule
\end{tabular}
\end{table}
