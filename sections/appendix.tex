...
\begin{table}[h]
\caption{Model components. Parameter counts are in millions. Shared weights are counted once. The frozen lookup is excluded from optimization and EMA updates.}
...
Denoising-active parameters & 90.44 & 638.15\\
Terminal-only decoder parameters & 156.52 & 157.05\\
Union of denoiser and decoder parameters & 246.96 & 795.21\\
...
\end{tabular}
\end{table}
The denoising-active count includes the shared Transformer, latent input/output projections, and time and SCCFG conditioning. Terminal-only parameters comprise the factored vocabulary head and decoder-mode tokens. The terminal decoder reuses the Transformer; it is not a separate 247M/795M model. A generation with $S$ integration steps makes $S+1$ ELF backbone calls, one prompt-Transformer call, and a prompt token lookup. SCCFG is supplied as learned conditioning and does not add an unconditional inference branch in this implementation. The lookup is a memory footprint, not a dense 389M-parameter computation at every step.
