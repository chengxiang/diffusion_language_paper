...
A separate identity-clock epoch-six extension evaluates the three teacher-prompt representation arms with four generation seeds (Table~\ref{tab:identityfourseed}). Learned minus PCA multilayer is $+3.07$ points with paired 95\% interval $[1.57,4.51]$; PCA multilayer minus layer 32 is $+10.94$ points, $[9.44,12.51]$. These intervals condition on the trained checkpoints and observed draws. They support the representation ordering under identity clocks and are not transferred to the two-seed shared-$t^2$ results.
...
\caption{Measured GPU inference time against total denoising steps from the earlier timing cohorts; the 8-step timing curve does not include the later expansion to 64 draws. Time includes prompt encoding, denoising, and terminal decoding, while excluding startup and scoring. These are throughput-derived amortized costs, not single-query latencies. ELF-B NFT runs pack two cases on one H200; the other panels use one case per H200.}
\label{fig:time}
\end{figure}
