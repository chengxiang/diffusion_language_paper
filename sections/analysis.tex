...
\paragraph{Guidance strength.}
The SCCFG sweep in Appendix~\ref{app:sccfgsweep} motivates an expanded MATH500 comparison of coefficients 2 and 3. At 64 steps with eight matched seeds, SCCFG 3 raises pre-NFT pass@1 from 18.85\% to 21.18\% ($+2.33$ points; paired 95\% interval $[1.27,3.43]$) and post-NFT pass@1 from 23.60\% to 24.68\% ($+1.08$; $[0.00,2.20]$). Pre-NFT gains have positive paired intervals at all four tested step counts; post-NFT effects are smaller and higher-$k$ coverage is mixed. This is a test-set sensitivity study at fixed checkpoints and seed lists. We retain the common SCCFG 2 headline protocol and report both settings in Appendix~\ref{app:mathsccfg3}.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/nft_improvement.pdf}
\caption{NFT pass@1 improvement over the final supervised checkpoint, in percentage points. Every update uses async $(2.5,2,1.5)$, 32 steps, SCCFG 2, and seeds 42/123. Bars are pointwise paired 95\% problem-bootstrap intervals, including uncertainty in the supervised reference. Stars mark development-selected updates; zero denotes no improvement. References are joint epoch 18 for ELF-B and epoch 21 for ELF-L. These two-seed curves are separate from the eight-seed, 64-step headline evaluation.}
\label{fig:nftimprovement}
\end{figure}
Figure~\ref{fig:nftimprovement} follows NFT at updates 100--600 while holding the evaluation protocol fixed. At the development-selected updates, gains over the final supervised checkpoint are $+4.21$ points for GSM8K ELF-B (paired 95\% interval $[2.73,5.69]$), $+3.22$ for GSM8K ELF-L ($[1.78,4.62]$), and $+5.10$ for MATH ($[2.50,7.70]$). The gains vary across updates, and several early MATH intervals include zero. This motivates distinguishing the selected endpoint from the entire post-training trajectory. Appendix~\ref{app:trainingcurves} includes the preceding supervised epochs and prompt substitution.
