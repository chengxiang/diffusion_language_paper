...
Let $\mathcal V$ be the vocabulary, $q=(q_1,\ldots,q_{L_q})$ the formatted prompt, and $a=(a_1,\ldots,a_T)$ its answer, including the terminal token. We use $j$ for a position within either sequence, $\ell$ for a teacher layer, and $d$ for latent width. An answer encoder $\mathcal E$ maps $(q,a)$ to clean latents $x=\mathcal E(q,a)$, with $x_j\in\R^d$ corresponding to $a_j$; padding is implicit. A prompt encoder separately maps $q$ to $c=P_\phi(q)\in\R^{L_q\times d}$ without access to the answer. Unlike a token embedding lookup $e_j=B_\psi[q_j]$, this encoding combines information across prompt positions. We reserve $\psi$ for frozen teacher parameters, $\phi$ for the learned prompt encoder, and $\theta$ for ELF. A decoder $p_{\theta,j}^{\mathrm{dec}}(v\mid x,c)$ assigns vocabulary probabilities at answer position $j$, enabling all answer tokens to be decoded in parallel.

\paragraph{Flow matching.}
Flow matching learns a velocity field transporting noise to data \citep{flowmatching}. We use a linear Gaussian path, oriented from noise at $t=0$ to clean answer latents at $t=1$:
\begin{equation}
 z_t=t x+(1-t)\sigma\epsilon,
 \qquad \epsilon\sim\mathcal N(0,I),\qquad
 \frac{\mathrm d z_t}{\mathrm d t}=x-\sigma\epsilon.
 \label{eq:methodinterpolant}
\end{equation}
Here $t$ is the corruption time, $\sigma>0$ the noise scale, and $z_t$ the noisy answer; the prompt remains clean. Sampling paired $(q,a)$, independent noise, and $t\sim\pi(t)$ gives the basic conditional objective
\begin{equation}
 \mathcal L_{\mathrm{FM}}^{\mathrm{basic}}(\theta,\phi)
 =\E\!\left[\left\|v_\theta\bigl(z_t,t,P_\phi(q)\bigr)
 -(x-\sigma\epsilon)\right\|_F^2\right],
 \label{eq:basicflowloss}
\end{equation}
where $\pi$ is the training-time sampling density and the Frobenius norm sums over answer positions and latent coordinates. Omitting the prompt condition recovers unconditional flow matching. Our masked, normalized objective and ELF-specific modifications are given in Section~\ref{sec:methodflow}.

\paragraph{What is the condition?}
The observed condition is the text $q$; $c=P_\phi(q)$ is an internal representation used by the model. Thus $V_{\theta,\phi}(z,t,q)=v_\theta(z,t,P_\phi(q))$ is one text-conditioned field, even when both networks are trained. Write $p_t(z\mid q)$ for the distribution induced by Eq.~\eqref{eq:methodinterpolant} and $\mathsf{s}_t(z\mid q)=\nabla_z\log p_t(z\mid q)$ for its conditional score. For $0<t<1$, the population-optimal field satisfies
\begin{equation}
 v_t^\star(z\mid q)
 =\E[x-\sigma\epsilon\mid z_t=z,q]
 =\frac{z+\sigma^2(1-t)\mathsf{s}_t(z\mid q)}{t}.
 \label{eq:conditionalflowscore}
\end{equation}
Conditioning only on an encoding $c$ instead gives the corresponding posterior given $c$, which can merge prompts with different answer distributions. The learning objective is therefore to preserve the information needed for conditional denoising, rather than any particular teacher coordinates. Section~\ref{sec:conditionalencoding} makes this distinction precise.

\paragraph{Sampling under a different clock.}
Inference integrates the learned field from Gaussian noise. Let $\tau\in[0,1]$ be a solver clock and $t=f(\tau)$ an increasing differentiable map with $f(0)=0$ and $f(1)=1$. The sampled state $\bar z_\tau$ follows
\begin{equation}
 \frac{\mathrm d\bar z_\tau}{\mathrm d\tau}
 =f'(\tau)V_{\theta,\phi}\bigl(\bar z_\tau,f(\tau),q\bigr),
 \qquad \bar z_0\sim\mathcal N(0,\sigma^2I).
 \label{eq:sharedclock}
\end{equation}
For $f(\tau)=\tau^2$, the multiplier is $2\tau$. An exactly integrated field has the same endpoint under either clock, but finite-step accuracy can change. Euler sampling evaluates the right-hand side on a grid $\tau_0,\ldots,\tau_K$; $k$ indexes solver steps and $K$ counts denoising evaluations. Changing this inference clock does not change the training density $\pi$. Section~\ref{sec:clocks} extends the shared clock to separate clocks for different latent groups.

\paragraph{Our fixed-canvas setup.}
Figure~\ref{fig:pipelineoverview} summarizes the pipeline. Each example occupies $N=1024$ text positions in total: prompt, answer, then padding. The latent width is separately $d=1024$. For full-canvas token labels $y_i$, define $M_i=1$ when $L_q<i\leq L_q+T$ and zero otherwise; $r$ indexes examples in a microbatch. Training uses $\sigma=2$ and one shared corruption time per example, restores clean prompt states, and scores only valid answer positions, including the terminal token. ELF alternates flow regression and same-position decoder cross-entropy with probabilities $0.8/0.2$ (Section~\ref{sec:methodflow}). At inference, the unknown answer region occupies all $N-L_q$ remaining positions, is denoised jointly, and is decoded in one terminal call; the first terminal token determines the returned answer length. Denoising NFE excludes this final decoding call. The ideal unguided field above supplies the background for ELF's practical self-conditioning, guidance, and endpoint stabilization.
...
\begin{equation}
 \widehat h_j^{(\ell)}=\mu_\ell+x_j^{(\ell)}D_\ell^\top.
...
\end{equation}
Away from the clamp, $v^*=x-\sigma\epsilon$ is the path velocity. For a microbatch of examples $r$ with response mask $M_{ri}$, the basic flow-matching loss is
\begin{equation}
 \mathcal L_{\mathrm{FM}}=
 \E\!\left[\frac{\sum_{ri} M_{ri}\|v_{\theta,ri}-v_{ri}^*\|_2^2}
 {d\sum_{ri} M_{ri}}\right].
...
A token head on the same Transformer backbone maps answer latents to vocabulary probabilities $p_\theta^{\mathrm{dec}}$. Decoder-mode training uses a separate corruption $z^{\mathrm{dec}}$ of the clean answer and same-position token labels $y_i$:
\begin{equation}
 \mathcal L_{\mathrm{CE}}=
 \E\!\left[-\frac{\sum_{ri} M_{ri}\log p_\theta^{\mathrm{dec}}(y_{ri}\mid z_r^{\mathrm{dec}},c_r)}
 {\sum_{ri} M_{ri}}\right].
...
Extending the text-conditioned field of Section~\ref{sec:preliminaries} to include answer self-conditioning $s$, we write
\begin{equation}
...
Let $X$ and $Q$ denote the clean answer latents and prompt, with $\E\|X\|_F^2<\infty$, and fix a deterministic encoder $C=P_\phi(Q)$. Let $\mathsf{s}_t(z\mid c)$ denote the score conditioned on $C=c$. Under the independent Gaussian corruption in Eq.~\eqref{eq:methodinterpolant}, for $0<t<1$ and $c=P_\phi(q)$,
\begin{equation}
\begin{split}
 \mathsf{s}_t(z\mid q)-\mathsf{s}_t(z\mid c)
 =\frac{t}{\sigma^2(1-t)^2}
 \bigl(&\E[X\mid Z_t=z,Q=q]\\
       &-\E[X\mid Z_t=z,C=c]\bigr).
\end{split}
...
\begin{equation}
 ...
 \qquad
 z_{\bm t}^{(\ell)}=t_\ell x^{(\ell)}+(1-t_\ell)\sigma\epsilon^{(\ell)}.
...
Write $z_k=\bar z_{\tau_k}$ for the sampled state and $\widehat x_k$ for its clean prediction at global time $\tau_k$. Each group follows its own endpoint-to-velocity conversion and clock derivative:
\begin{equation}
 z_{k+1}^{(\ell)}=z_k^{(\ell)}+
...
\[
 \lambda_{ri}=\operatorname{sigmoid}(0.8+0.8\xi_{ri}),\qquad
 z^{\mathrm{dec}}_{ri}=\lambda_{ri}x_{ri}+(1-\lambda_{ri})\epsilon'_{ri},
\]
where $\xi_{ri}$ and $\epsilon'_{ri}$ are standard Gaussian. Decoder noise scale is one. The model receives decoder mode, time one, zero answer self-conditioning, and the clean question prefix. Token CE predicts the original token at each position, without a next-token shift.

For valid response mask $M_{ri}$, including the terminal token and excluding question/padding positions, a rank-local microbatch minimizes
\begin{equation}
 \mathcal L_{\mathrm{ELF}}^{\mathrm{micro}}=
 \frac{\sum_{ri}M_{ri}\left[
 (1-b_r)\|v_{\theta,ri}-\widetilde v^*_{ri}\|_2^2/d
 -b_r\log p_{\theta,ri}^{\mathrm{dec}}(y_{ri})\right]}
 {\sum_{ri}M_{ri}}.
...
For $K$ denoising steps, we set $\tau_0=0$, $\tau_K=1$, and use the same logit-normal-quantile grid across schedules:
\begin{equation}
 \tau_k=\operatorname{sigmoid}\!\left[-1.5+0.8\Phi^{-1}(k/K)\right],
 \qquad 0<k<K.
\end{equation}
