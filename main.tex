...
Continuous diffusion generates embeddings or latents, whereas discrete diffusion reverses categorical corruption, as in masked-token generation \citep{plaid,mdlm,llada}. Continuous reasoning models include simplex-based TESS~2 and hyperspherical $\mathbb S$-FLM \citep{tess2,hyperspherical}. ELF denoises contextual representations with a shared denoiser and decoder; the concurrent ELF-REG adds alignment to teacher features and a jointly denoised global representation \citep{elf,scalingdlm}. Both use an external contextual encoder for conditional generation. We retain ELF's foundation while learning the answer representation and progressively replacing teacher conditioning with a trainable prompt encoder. Few-step approaches include flow maps and token refinement (FMLM+, MLFM) and fixed-point transport (DBTM) \citep{posterior,mlfm,dbtm}. PlaidQ distills continuous code generation into few-step and one-step models \citep{plaidq}, while ELF-REG uses early-stop decoding without dedicated few-step training \citep{scalingdlm}.
