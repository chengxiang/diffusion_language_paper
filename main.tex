...
\paragraph{Continuous and discrete language generation.}
Continuous diffusion models generate real-valued token embeddings or contextual latents and subsequently decode them into text. Discrete diffusion models instead reverse corruption of categorical token states, often through masking and unmasking. These are distinct generative constructions; a continuous time variable alone does not make a model continuous-state. Plaid develops likelihood-based diffusion over learned token embeddings \citep{plaid}, whereas MDLM and LLaDA develop and scale masked-token generation \citep{mdlm,llada}. Continuous models have also begun to support reasoning: TESS~2 adapts an autoregressive model to simplex diffusion and instruction following \citep{tess2}, and $\mathbb S$-FLM studies reasoning with flows over hyperspherical token embeddings \citep{hyperspherical}. We focus on generating complete reasoning responses through continuous denoising, with discrete models providing a separate performance reference.

\paragraph{Reasoning and generation with few denoising steps.}
Recent methods improve the quality attainable with a short generation trajectory through different mechanisms. FMLM+ combines learned flow maps with masking-style noise schedules and posterior refinement, retaining confident tokens while regenerating others \citep{posterior}. MLFM similarly combines continuous denoising with discrete token promotion \citep{mlfm}. DBTM learns an autonomous transport map with a fixed-point objective and extends it to partial-context refinement \citep{dbtm}. PlaidQ initializes a continuous embedding denoiser from an autoregressive model and uses distribution-matching and paired-trajectory distillation for few-step and one-step code generation \citep{plaidq}. ELF-REG instead decodes an intermediate clean prediction to obtain low-NFE reasoning without dedicated few-step training \citep{scalingdlm}. Our work studies how representation construction and denoising schedules support reasoning under the standard flow-matching and decoder-CE recipe. Tables~\ref{tab:headlinecomparison}--\ref{tab:headlinelownfe} compare author-reported results while distinguishing model size and denoising budget.

\xc{1. We should mention that ELF-REG is concurrent work. 2. Due to space, maybe list the few-step methods in 1-2 sentences instead of in a single paragraph. 3. best to mention ELF and ELF-REG together, and mention that these do conditional generation with an external conditional encoder.}

\paragraph{Learning the space in which denoising occurs.}
ELF establishes contextual encoder activations as useful language diffusion targets and shares a Transformer between denoising and token decoding \citep{elf}; we retain this foundation. Representation learning can also act at different points in the generative pipeline. REPA supervises intermediate denoiser features using pretrained visual representations \citep{repa}. ELF-REG transfers this principle to language, aligning intermediate features to an autoregressive teacher and jointly denoising an additional global teacher representation \citep{scalingdlm}. TextLDM instead aligns a text VAE encoder with a frozen Qwen teacher before training latent diffusion; its experiments show that reconstruction fidelity alone does not determine generation quality \citep{textldm}. LDLM jointly learns a latent encoder, denoiser, and decoder, using a warmup to stabilize the diffusion objective's influence on the latent space \citep{ldlm}. These findings motivate examining both decodability and suitability for denoising. Our construction learns compact projections of several teacher layers using downstream token prediction, then fixes this answer representation during flow training. The reasoning ablations in Section~\ref{sec:representation} examine how layer choice and projection learning affect the resulting generator.

\xc{again, above is too long. Try to compress, but worth keeping the parts which motivate our own investigation (decodability vs suitability for denoising, etc).}

\paragraph{Structured representations and denoising order.}
In image generation, Semantic-First Diffusion couples semantic and texture latents with separate noise schedules, while Latent Forcing jointly generates latents and pixels with different denoising trajectories \citep{sfd,latentforcing}. These approaches connect the organization of a representation to when its components become informative. We investigate this connection within language: features from different depths of an autoregressive teacher form separate groups of each token's latent vector. Local clocks then control the relative denoising progress of these groups. Our schedule comparisons and reconstruction probes examine the role of this decomposition (Section~\ref{sec:clocks}); the ordering is across representation groups at every answer position, rather than across token positions.

\paragraph{Learning the conditioning interface.}
A contextual prompt encoder uses interactions among prompt tokens, whereas a token embedding table maps each token independently. ELF uses a frozen T5 encoder, and ELF-REG uses frozen Qwen features for conditioning \citep{elf,scalingdlm}. FMLM+ can instead condition directly on clean token states within its flow-map model \citep{posterior}. We study how to replace a pretrained contextual encoder while preserving the conditioning structure that supports reasoning. Our curriculum first trains the flow under fixed teacher conditioning, then substitutes an imitation-trained prompt encoder and jointly adapts the two models (Section~\ref{sec:conditionalencoding}). This is related to LDLM's stabilization of coupled representation learning \citep{ldlm}, but concerns the conditioning pathway while the answer latent space remains fixed. The comparison with joint training from initialization motivates studying \emph{when} a conditioning representation should become learnable.

\xc{to save space, do not list a separate section. We can perhaps mention the conditioning when first referenceing these papers, instead of in a separate dedicated section.}

\paragraph{Reward-based post-training.}
Diffusion reasoning already benefits from reward and preference optimization: d1 applies policy-gradient training to masked diffusion, and LLaDA~1.5 improves preference optimization through variance reduction \citep{d1,llada15}. LaDi-RL explores reinforcement learning for continuous latent reasoning trajectories followed by text decoding \citep{ladirl}. DiffusionNFT provides a complementary route for continuous generators, using forward-process flow matching on rewarded samples without estimating reverse-trajectory likelihoods \citep{diffusionnft}. We adapt this objective to a full-response latent generator whose rollout field uses learned self-conditioning guidance, and jointly optimize its prompt encoder. Section~\ref{sec:nft} develops this adaptation and gold-solution anchoring; Section~\ref{sec:nftresults} examines improvements in correctness alongside changes in multi-sample coverage and majority-vote accuracy.

\xc{just 1 sentence listing diffusion RL papers, and one more to single out NFT, since we use it.}
...
\xc{The conventional notation for token position index etc in this paper should be established in the preliminary/background section too.}

Let $q$ denote the question prefix, $a=(a_1,\ldots,a_T)$ its solution, and $h_j^{(\ell)}\in\R^{2560}$ the frozen teacher's post-block residual at answer position $j$. The native causal mask gives $h_j^{(\ell)}=h_\psi^{(\ell)}(q,a_{\leq j})$; extracting all these states requires a single forward pass over $(q,a)$. For $\mathcal S=\{16,24,32\}$, we learn a linear encoder $E_\ell\in\R^{2560\times d_\ell}$ and form
\begin{equation}
...
We compare ELF-B models trained for six epochs on the same 2.43M-row GSM8K training population, with the same ELF initialization, optimization batch, and synchronous training clocks. All representations have width 1,024 and use covariance whitening. The main comparisons use original Qwen question conditioning, holding the representation fixed during ELF training. We evaluate with a common $t=\tau^2$ clock \xc{clock/schedule and denoising should also be explained in the preliminary section}, 32 denoising steps, and ELF EMA $0.999$. The fourth arm in Figure~\ref{fig:representationcomparison} is for a separate but related ablation that is discussed later in \xc{Section ???}.
...
% BEGIN inlined sections/methodology.tex
\section{Methodology}
\label{sec:methodology}

\xc{section title should be improved}

\xc{I think the introduction to the flow-setting should come before we discuss detailed representation construction. Perhaps we should have a separate section, right after intro, where we talk about "preliminary/background", where we introduce the basic flow matching setup, as well as the basic setup specific to this paper "conditional decoding over a block of 1024 tokens".}

With the answer representation fixed, we learn a conditional generator over complete reasoning traces. Our denoising and token-decoding objectives closely follow ELF \citep{elf}. The principal changes concern the representation being generated and how the prompt is encoded and introduced during training (Figure~\ref{fig:pipelineoverview}).
...
\end{equation}
The path starts at noise and ends at the teacher-derived answer. One time is drawn per training example and shared across its tokens and layer groups. Question positions remain clean; a bidirectional Transformer jointly processes the prompt and noisy answer.
...
We view the prompt encoder as an internal component of a model conditioned on question tokens. \xc{we should preface with a higher-level conceptual discussion somewhere. Perhaps after we set up notation in preliminary section for conditional score.} Writing its composition with the denoiser as
\begin{equation}
...
\xc{This lemma is way unnecessarily abstract. We should specialize to scores/conditional scores, which is what we really care about here. You can put the more abstract lemma in the appendix as a step towards proving the more concrete lemma.}
...
The proof is given in Appendix~\ref{app:conditioninglemma}. \xc{Applied to the target in Eq.~\eqref{eq:methodvelocity}, the lemma makes posterior prediction the criterion for a sufficient conditional representation. Exact agreement with the teacher's latent coordinates is unnecessary: a jointly adapted denoiser can interpret a different encoding with the same relevant information. The above is a bit hard to understand. Perhaps we can give an example: even if the condition is perturbed, if the neural network learns the ideal score and denoising is exact, we still recover the target distribution $p$ exactly at the end of denoising.} The identity permits noise-dependent encoding error but makes no assumption that higher noise is more forgiving. It describes the population regression objective; the effectiveness of learning this interface with finite models depends on optimization. Our curriculum therefore separates establishing the conditional generator from learning its conditioning interface:
\begin{enumerate}
 \item \textbf{Train ELF under frozen Qwen conditioning.}
 For the first 12 ELF epochs, use the projected question-only teacher states $c_{\mathrm T}(q)$ \xc{slightly confusing notation} and optimize the flow/decoder objective. The teacher and representation projectors remain frozen, giving the denoiser a stable, informative condition throughout its initial training.
...
\xc{change title to "Importance of Fixing Initial Conditioning"}
...
% END inlined sections/methodology.tex
